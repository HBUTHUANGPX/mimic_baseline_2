...
def build_root_state(root_pos: np.ndarray, root_quat: np.ndarray, env_origins: np.ndarray) -> np.ndarray:
...
def prepare_robot_cfg(robot_cfg):
    ...
    if hasattr(prepared, "spawn") and hasattr(prepared.spawn, "activate_contact_sensors"):
        prepared.spawn.activate_contact_sensors = False
    return prepared


def parse_args():
    parser = argparse.ArgumentParser(description="Replay retargeted motion npz files with Isaac Lab.")
    parser.add_argument(
        "--motion_file",
        type=str,
    ...
    parser.add_argument("--camera_distance", type=float, default=3.0, help="Camera distance behind the root.")
...
def main():
    ...
    scene_cfg = ReplaySceneCfg(num_envs=args_cli.num_envs, env_spacing=args_cli.env_spacing)
    ...
    while simulation_app.is_running():
        ...
        root_pos = np.repeat(motion.robot_root_pos[frame_idx][None, :], scene.num_envs, axis=0)
        root_quat = np.repeat(motion.robot_root_quat[frame_idx][None, :], scene.num_envs, axis=0)
        root_state = torch.from_numpy(build_root_state(root_pos, root_quat, env_origins)).to(sim.device)
        robot.write_root_state_to_sim(root_state)

        joint_pos = robot.data.default_joint_pos.clone()
        joint_vel = torch.zeros_like(robot.data.default_joint_vel)
        joint_pos[:, joint_indices] = robot_joint_pos[frame_idx].unsqueeze(0).repeat(scene.num_envs, 1)
        ...
        sim.set_camera_view(pos_lookat + np.array([args_cli.camera_distance, args_cli.camera_distance, 0.5]), pos_lookat)
        time_step += 1
